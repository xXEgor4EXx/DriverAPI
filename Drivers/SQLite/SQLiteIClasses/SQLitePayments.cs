...
class DBPayments : IDBPayments
{
     private Payments FromReader(SqliteDataReader reader)
    {
        return new Payments
        {
            ...
            AmountToPay = (float)reader.GetDouble("WorkID"),
    ...
    {
          string query = "SELECT PaymentID, EmployeeID, AmountToPay, PaymentDate FROM Operations WHERE PaymentID = @PaymentID";
        var parameters = new List<SqliteParameter>
        {
    ...
    {
        string query = "SELECT PaymentID, EmployeeID, AmountToPay, PaymentDate FROM PaymentID";
